...
first and reads the other two. [upgrading](docs/upgrading.md#upgrading-from-075)
has the same list with what each one applies to.

- **Regenerate** (`rake graph_weaver:generate`): generated `execute` now runs
  under the per-call `operation.graph_weaver` event, and a shared fragment on
  an interface or union that selects no `... on`, or exactly one, hoists into
  `GraphQLTypes` under the fragment's name. A module generated by 0.7.5 keeps
  working; it emits no operation event and no info log line until regenerated,
  and app code naming a struct that moved is found by `srb tc`.

- **Read the one Rails log line as the caller's outcome.** It is the
  operation's now: `failed GraphWeaver::CastError` where it said `ok`, present
  under a `graphql:` tag, one line for a retried call. Anything parsing the
  per-attempt `[502]` / `(retry 2)` line finds it at debug.

- **Let CI reach every graph's server.** `queries:check` re-introspects a named
  graph whose dump records a url, or whose client names a server or runs a
  schema class, where it used to check the committed file. A single-schema app
  already needed that reach.
...
- **`bin/generate` regenerates the GitHub example too.**
  `examples/github/generated` was regenerated by no build step and gated by
  none, so it shipped two releases behind the emitter that wrote it. It now
  moves with everything else generated: `bin/generate` rewrites it from the
  schema dump the example caches, and says in one line that it skipped it when
  there is no dump — the offline gate never reaches the network. The nightly
  integration workflow re-derives it against GitHub's live schema and fails on
  a diff, which is the half of the rule that runs where the network is.

- **One instrumentation event per CALL, above the one per request.**
  `operation.graph_weaver` (`GraphWeaver::OPERATION_EVENT`) covers one
  `execute`/`execute!` of a generated module start to finish — the request it
  made, every retry *and its backoff*, and the cast into your structs — with the
  `execute.graph_weaver` events nesting inside it. It is the only event that
  says what the *caller* got: a `CastError` is raised after the response is
  back, so the request event had already closed `:ok` while the app saw a
  failure. And it fires at the module seam, which every client slot passes
  through, so a fake, the test router and a cassette report too — a spec can
  assert on instrumentation under any `graphql:` tag. Payload: `:operation`,
  `:module`, `:graph`, `:kind`, `:client`, `:status`, `:duration_ms`, plus
  `:code` or `:error`; the attempt facts (`:url`, `:http_status`, `:retries`)
  stay on the request event, since a call that took three goes has no one
  answer for any of them. **The one Rails log line at info is the operation's
  now** — it reads `failed GraphWeaver::CastError` where it used to read `ok`,
  it appears under a `graphql:` tag, and a retried call is one line instead of
  one per attempt; the per-attempt line, with its `[502]` and `(retry 2)`,
  writes at debug.
  **Action: Regenerate** (`rake graph_weaver:generate`) — a module generated by
