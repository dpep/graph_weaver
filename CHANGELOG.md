...
- **Every instrumentation event says which graph the request went to.** The
  ...
  groups an APM dashboard by graph without parsing operation names, and the
  Rails log line reads `GraphWeaver billing/InvoicesQuery (12.3ms) ok`
  (unprefixed when there's no graph, so a single-graph app's log is unchanged).
  The label covers **one request**: a generated `execute` labels the request it
  makes — each one, when a federated operation fans out — and never what a
  *server* does while answering, so an in-process resolver that calls another
  API produces an event carrying its own graph, or none, rather than its
  caller's. It is read from what codegen baked in, never inferred from the
  client: a wrong graph on a request is worse than no graph.
  ([logging](docs/logging.md#the-payload))
- **A generated `execute` makes its request through the gem.** The body is now
  `from_response(dispatch(variables, client:))` — `GraphWeaver::QueryModule`
  owns the call, reading the module's own `QUERY`/`OPERATION_NAME`, which is
  what lets `:graph` above bracket a request without a line of it landing in
  every generated file. Behavior, sigs and the `Response` envelope are
  identical, and modules generated by 0.6.1 keep working as they are — but
  `rake graph_weaver:verify` reports the tree out of date until you regenerate,
  so run `rake graph_weaver:generate` with the upgrade.
