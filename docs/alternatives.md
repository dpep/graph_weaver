...
- **No `@defer`, no file uploads, no persisted queries, no batching.** artemis
  ...
  none of the four.
- **No instrumentation seam.** `GraphWeaver.logger` is the whole observability
  story — no `ActiveSupport::Notifications`, no hook for an APM. graphql-client
  emits `query.graphql` and `error.graphql` events, and artemis wires a log
  subscriber for you.<!-- lib/graphql/client.rb:362,381; lib/artemis/railtie.rb -->
  For a gem that ships a railtie, that's the notable omission.
