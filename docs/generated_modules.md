...
in the output directory are safe. A run that finds **no** queries says where it
looked rather than exiting 0 in silence, and `verify_generated!` fails outright:
a mistyped `queries_paths` used to leave a CI gate green forever.
