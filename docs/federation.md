...
| a nested `@requires` field set no one subgraph holds | a nested field set crosses as one object, so one fetch has to answer the whole of it. Nesting itself is fine — this is the set whose fields are split across subgraphs, which would take a fetch per level |
