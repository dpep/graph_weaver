...
`0.7.5` published to RubyGems (2026-09-20), after `0.7.4` (2026-09-17),
`0.7.3` (2026-09-16), `0.7.2` (2026-09-15), `0.7.1` (2026-09-14) and `0.7.0`
(2026-09-13). 0.7.0's CHANGELOG entry is the diff from 0.6.1 — multi-graph
...
inherited dump, and `docs/migrating.md`. The upgrade guide has a checklist per
step; every entry marked **Regenerate** changed emitted code. `research/`
holds the user-test logs and the two design memos (migration, naming).
...
6. **Smaller:** `examples/github/generated/**` is regenerated by nothing and read by
   no spec, so it drifted two releases before a scribe noticed — fold
   `examples/github/generate.rb` into `bin/generate` behind a token, or gate the
   files with a parity spec; a correlation id in the payload; two cold clients
   both writing the conventional dump once. Declined after a look:
