...
6. **Smaller:** `examples/github/generated/**` is regenerated by nothing and read by
   ...
   files with a parity spec; `:wire` refuses the whole example when any declared graph's
   ...
   `source_transport`'s message for a supergraph; `null_chance` per coordinate like
